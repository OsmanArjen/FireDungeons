...
def from_folder(folder : str):
    ...
    images = []
    for file in Files:
        images.append(pg.image.load(os.path.join(folder,file)).convert_alpha())
    return images
def scale2x_from_folder(folder: str):
    Files = sorted(os.listdir(folder))
    images = []
    for file in Files:
        images.append(pg.transform.scale2x(pg.image.load(os.path.join(folder,file))).convert_alpha())
    return images
def scale_from_folder(folder: str,scale):
    Files = sorted(os.listdir(folder))
    images = []
    for file in Files:
        images.append(pg.transform.scale(pg.image.load(os.path.join(folder,file)),scale).convert_alpha())
    return images
